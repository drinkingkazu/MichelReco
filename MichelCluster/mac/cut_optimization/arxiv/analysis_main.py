...
# plt.show()
Ldata = np.array(Ldata) - data.total_sig

import ROOT
c1   = ROOT.TCanvas()
th2d = ROOT.TH2D("lowest_mean_chi",
                 ";Lowest #chi^{2};Mean #chi^{2}",
                 100,0,1,100,0,50)

for i in xrange(cut1Vars.size):
    for j in xrange(cut2Vars.size):
        th2d.SetBinContent(i+1,j+1,Ldata[j][i])

ROOT.gStyle.SetPalette(55)
#th2d.SetContour(1000)
th2d.Draw("COLZ")
...
tf = ROOT.TFile("outfile2.root","RECREATE")
